...
stability_cluster <- function(data, k, n_iter = 100L, r_seed = 1234) {
  ...
  c_mat <- parallel::mclapply(1:n_iter, function(i) {
    # set seed here and turn off mc control
    s <- withr::with_seed(
      i + r_seed, sample(n, floor(n / 2), replace = FALSE))
